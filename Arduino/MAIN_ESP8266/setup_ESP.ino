boolean setup_ESP(){//returns a '1' if successful
  ...
  if(read_until_ESP(keyword_OK,sizeof(keyword_OK),10000,0))//go look for keyword "OK" with a 5sec timeout
  ...
  if(read_until_ESP(keyword_OK,sizeof(keyword_OK),10000,0))//go look for keyword "Ready" - takes a few seconds longer to complete
  ...
  if(read_until_ESP(keyword_OK,sizeof(keyword_OK),10000,0))//go look for keyword "OK"
    ...
    Serial.println(F("ESP CWMODE SET FAILED")); //probably going to fail, since a 'no change' is returned if already set - would be nice to check for two words
  serial_dump_ESP();  
  ...
  Serial.println(F("CHECKING FOR AN IP ADDRESS"));
...
//size of teh keyword, but remember, sizeof() will return one extra, which is going to be subtracted, so I just added it back in +1
    ...
    Serial.print(F("IP ADDRESS = "));//print it off to verify
    for(int i=1; i<=ip_address[0]; i++)//send out the ip address
    Serial.print(ip_address[i]);
    Serial.println("");
  }}//if first \r\n
  else
  Serial.print(F("IP ADDRESS FAIL"));
  serial_dump_ESP();
   ...
   ESP8266.print("\r\n");
  if(read_until_ESP(keyword_OK,sizeof(keyword_OK),10000,0))//go look for keyword "OK" or "no change
    Serial.println(F("ESP CIPMUX SET"));
  else
    Serial.println(F("ESP CIPMUX SET FAILED")); 
  serial_dump_ESP();
