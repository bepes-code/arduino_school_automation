boolean connect_ESP(){//returns 1 if successful or 0 if not
  ...
  if(read_until_ESP(keyword_OK,sizeof(keyword_OK),10000,0)){//go look for 'OK' and come back
  ...
  if(read_until_ESP(keyword_carrot,sizeof(keyword_carrot),10000,0)){//go wait for the '>' character, ESP ready for the payload
    ...
    */
    ...
    if(read_until_ESP(keyword_sendok,sizeof(keyword_sendok),10000,0)){//go wait for 'SEND OK'
